...
const ASSET_VERSION = '20260908-phonefix'
...
const HERO_PHONE_VIDEO_SRC = asset('assets/hero-video-phone.mp4')

const isPhoneLikeDevice = () => {
  if (typeof window === 'undefined') return false
  const ua = navigator.userAgent || ''
  const touchPhone = window.matchMedia?.('(hover: none) and (pointer: coarse)').matches
  const smallPhysicalScreen = Math.min(window.screen?.width || 9999, window.screen?.height || 9999) <= 820
  return /Android|iPhone|iPod|Windows Phone|Mobile|MicroMessenger/i.test(ua) || (touchPhone && smallPhysicalScreen)
}

const getHeroVideoSource = () => (isPhoneLikeDevice() ? HERO_PHONE_VIDEO_SRC : HERO_VIDEO_SRC)
...
function App() {
  ...
  useEffect(() => {
    ...
    document.addEventListener('WeixinJSBridgeReady', retryPlay, { once: true })
    return () => {
      window.removeEventListener('touchstart', retryPlay)
      document.removeEventListener('WeixinJSBridgeReady', retryPlay)
    }
  }, [])
